// SHFE_INEView.cpp : implementation file
//

#include "stdafx.h"
#include "../App.h"
#include "SHFE_INEView.h"
#include <float.h>
#include "../../ServiceLayer/CtpMsg.h"

// CSHFE_INEView

IMPLEMENT_DYNCREATE(CSHFE_INEView, CView)

CSHFE_INEView::CSHFE_INEView()
{
	m_pGridCtrl = NULL;
}
...
BEGIN_MESSAGE_MAP(CSHFE_INEView, CView)
	ON_WM_SIZE()
	ON_MESSAGE(WM_UPDATE_GRIDCTRL, &CSHFE_INEView::OnUpdateGridCtrl)//将消息与消息映射函数相绑定
END_MESSAGE_MAP()


// CSHFE_INEView drawing

void CSHFE_INEView::OnDraw(CDC* pDC)
{
...
// CSHFE_INEView diagnostics

#ifdef _DEBUG
void CSHFE_INEView::AssertValid() const
{
	CView::AssertValid();
}

#ifndef _WIN32_WCE
void CSHFE_INEView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}

void CSHFE_INEView::OnReceiveData(WPARAM wParam, LPARAM lParam)
{
...
void CSHFE_INEView::OnInitialUpdate()
{
	CView::OnInitialUpdate();
	CString title = _T("上海/能源");
	...
	if (m_pGridCtrl == NULL)
	{
		...
		vector<ColumnInfo> header_colums = cfg_reader.GetHeaderColumn("ListCtrl报价表");//根据表名,获取表内各列列头结构体ColumnInfo,合计为vector
...
// CSHFE_INEView message handlers


void CSHFE_INEView::OnSize(UINT nType, int cx, int cy)
{
...
LRESULT CSHFE_INEView::OnUpdateGridCtrl(WPARAM wParam, LPARAM lParam)
{
	//SHFE、INE
	...
	{//接受上海及能源交易所的合约数据，并做重排序
		...
		if (instruments_list != NULL)
		{
			...
			{//只接受上海及能源交易所的合约数据
				...
				if (Exchange_id == _T("SHFE") || Exchange_id == _T("INE"))
				{
...
void CSHFE_INE_ChildView::UpdateMarketDataView(LPARAM lParam)
{
